/*
 Name:		Humpty_Dumpty2.ino
 Created:	5/24/2017 10:30:54 PM
...
//these hold values used by the velocity encoders to perform a specific maneuver.
int turnticksleft = 11;
int turnticksright = 10;
int fullturnright = 21;
int fullturnleft = 20;
int normspeed = 230;	//speed of normal activity
int setspeed = normspeed;		//speed used in motion functions. Change this to norm and ramp speed as needed.
int rampspeed = 255;	//increase in power for climbing the ramp.
//NOTE: To high of a setpeed may cause line sensors to not detect a line in time and either overcorrect or not correct at all.
int rotspeed = 255;
int reversepeed = 200;
int correction = 4;
int bigcorrection = 6;
...
objective redpapproach = { 18,1,90 };
...
//move bot to the passed objective.
void goto_objective(struct objective obj)
{
	...
	// turn to either 90 or 270 degrees to do the X axis first
	if (intersectsx < 0)
	{
	...
	if (intersectsy < 0) {			//turn to either 0 or 180 to degreees for the y part of the objective
...
void followline(int sects) //parameters are the number of intersections where an intersection is a cross formed by two white lines
{
	...
	while (intersects <= sects)//less than or equal to2 because it likely started off on an intersect. 
	{
		...
		else if (!lo && !ro) //if both outer sensor are untriggerd...
		{
			if (!li && !ri) //if both inner ones are untriggered... 
			{
				moveforward();
				digitalWrite(Rled, 0);
				digitalWrite(Gled, 0);
		...
		else if (!lo || !ro) // if only one of the outer ones are triggered, then we are still off the line and we need to correct.
		{
			if (!li && !ri)
			{
				moveforward();
...
void dropOff(int binPos) {
	if (binPos > 0) {
		...
		moveServo(panServo, panServo.read(), 130);				//pan to the side
...
void loop()
{
	...
	setspeed = rampspeed; // increase in power to climb ramp with ease
	...
	setspeed = normspeed; //reset power at end of ramp to a more maneagable value. should be changed if setspeed is changed in decleration.
	
	followlinedistance(boxdistance);
	//dropOff(0);
	dropOff(bin2Pos);
	dropOff(bin1Pos);
	reverse(boxdistance - 1);
